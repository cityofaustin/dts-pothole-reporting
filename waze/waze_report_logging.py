...
def retrieve_stored_active_potholes(soda_client):
    active = soda_client.get(POTHOLE_LOG_DATASET, select="uuid", where="status == 'active'")
...
def mills_to_socrata_timestamp(mills, local_tz="America/Chicago"):
    if mills and type(mills) == int:
        ...
        central_time = datetime.fromtimestamp(mills / 1000.0, tz=ZoneInfo(local_tz)).strftime("%Y-%m-%dT%H:%M:%S")
        utc_time = datetime.fromtimestamp(mills / 1000.0, tz=ZoneInfo("UTC")).strftime("%Y-%m-%dT%H:%M:%SZ")
...
def main():
    ...
    for uuid in active_uuids:
        if uuid not in current_uuids:
            updates_to_socrata.append({"uuid": uuid, "status": "archived"})

    # 4. clean up the data for currently active pothole reports
    for record in data["alerts"]:
        record["status"] = "active"
        record["location"] = create_wkt_point(record["location"]["x"], record["location"]["y"])
        record["published_datetime_local"], record["published_datetime_utc"] = mills_to_socrata_timestamp(record["pubMillis"])
        record["reportByMunicipalityUser"] = string_to_boolean(record["reportByMunicipalityUser"])
